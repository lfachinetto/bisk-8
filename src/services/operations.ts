...
export function searchInstruction(registers: RegisterBank, memory: number[], step?: number): string[] {
  // 0	MAR <- PC
  // 1	MDR <- MEM[MAR]
  // 2	PC <- PC + 1
  // 3	IR <- MDR
  ...
  if (step == 1 || step == null) {
    registers.findByName('MDR')!.value = memory[registers.findByName('MAR')!.value];
...
export function searchAddress(registers: RegisterBank, memory: number[], step?: number): string[] {
  // 4	MAR <- PC
  // 5	MDR <- MEM[MAR]
  // 6	PC <- PC + 1
  const rtl: string[] = [];
...
export function add(registers: RegisterBank, memory: number[], step?: number): string[] {
  // 7	MAR <- MDR
  // 8	MDR <- MEM[MAR]
  // 9	ACC <- ACC & MDR
  const rtl: string[] = [];
